...
def output_result(result):
    """خروجی استاندارد JSON با تضمین ارسال کامل"""
    try:
        json_str = json.dumps(result, ensure_ascii=False, indent=2)

        # ارسال یکجا به جای خط به خط (کاهش overhead)
        output = f"---RESULT_START---\n{json_str}\n---RESULT_END---\n"

        # استفاده از write به جای print برای کنترل دقیق‌تر
        sys.stdout.write(output)
        sys.stdout.flush()

        # ⭐ فلش سطح سیستم‌عامل (اگر امکان‌پذیر باشد)
        try:
            os.fsync(sys.stdout.fileno())
        except:
            pass

        # ⭐ تاخیر طولانی‌تر برای payload بزرگ (12 کارت با metadata کامل)
        sleep(1.5)  # افزایش از 0.8 به 1.5 ثانیه

    except Exception as e:
        error_result = {
            "success": False,
            "error": f"Output serialization failed: {str(e)}",
            "timestamp": datetime.now().isoformat()
        }
        sys.stdout.write(f"---RESULT_START---\n{json.dumps(error_result, ensure_ascii=False)}\n---RESULT_END---\n")
        sys.stdout.flush()
        sleep(0.5)

def output_result_to_file(result):
    """خروجی JSON به فایل (برای داده‌های بزرگ)"""
    import tempfile
    try:
        ...
        # اعلام مسیر فایل به جای محتوای JSON
        flush_print(f"---RESULT_FILE---")
        flush_print(temp_path)
        flush_print(f"---RESULT_END---")
        sys.stdout.flush()
        sleep(0.5)
    except Exception as e:
        flush_print(f"---RESULT_START---")
        flush_print(json.dumps({"success": False, "error": str(e)}))
        flush_print(f"---RESULT_END---")
...
def main():
    ...
    try:
        ...
        if args.base_id is None:
            ...
            # ⭐ خروجی JSON و تضمین ارسال کامل
            output_result(result)

            # ⭐ تاخیر اضطراری قبل از disconnect
            sleep(0.3)
            disconnect(seller_bot)

            # ⭐ تاخیر نهایی قبل از خروج
            sleep(0.5)
            return 0
            ...
            # ⭐ دیباگ: نمایش پارامترها و کارت‌های موجود
        ...
        # ⭐ تاخیر قبل از disconnect در حالت خطا
        sleep(0.3)
        if seller_bot:
            disconnect(seller_bot)
        if buyer_bot:
...
if __name__ == "__main__":
    ...
    sys.stdout.flush()
    sleep(1.0)
    sys.exit(exit_code)  # ✅ فقط یکجا sys.exit صدا زده می‌شه
