...
export async function POST(req: NextRequest) {
    ...
    const { sellerKey, buyerKey, power, count, maxBids = 7, taskId = Date.now().toString() } = body;

    if (!sellerKey || !buyerKey || !power || !count) {
        return new Response(JSON.stringify({ error: 'Missing required parameters' }), { status: 400 });
    }

    const scriptPath = path.join(process.cwd(), 'scripts', 'cardTransfer', 'main.py');
